...
import hundsun.pdpm.domain.GenConfig;
...
public class GenUtil {
    ...
    public static void generatorCode(List<ColumnInfo> columnInfos, GenConfig genConfig,String tableCode, String tableName) throws IOException {
        ...
        for (ColumnInfo column : columnInfos) {
            ...
            String changeDictName =StringUtils.isEmpty(StringUtils.toCapitalizeCamelCase(column.getDictName()))?null:"dict"+StringUtils.toCapitalizeCamelCase(column.getDictName());
            if(!StringUtils.isEmpty(column.getDictName())){
                if(!isHasDict){
                    isHasDict = true;
                    dictJoint = "'"+column.getDictName()+"'";
                }else {
                    dictJoint += ",'"+column.getDictName()+"'";
                }
                dictRefJoint+=" :"+changeDictName+"=\"dict."+column.getDictName()+"\"  ";
            }
            listMap.put("dictName",column.getDictName());
            listMap.put("changeDictName",changeDictName);
        ...
        map.put("isHasDict",isHasDict);
        map.put("dictRefJoint",dictRefJoint);
        map.put("dictJoint",dictJoint);
